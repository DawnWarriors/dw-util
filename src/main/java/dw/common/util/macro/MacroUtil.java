...
public class MacroUtil {
    //日期替换
    private static String CURYEAR = "\\$\\{CURYEAR\\}";            //当前年（四位）
    private static String CURYEAR2 = "\\$\\{CURYEAR2\\}";            //当前年（两位）
    private static String CURMONTH = "\\$\\{CURMONTH\\}";            //当前月
    private static String CURDAY = "\\$\\{CURDAY\\}";            //当前日
    private static String CURHOUR = "\\$\\{CURHOUR\\}";            //当前时
    private static String CURMINUTE = "\\$\\{CURMINUTE\\}";        //当前分
    private static String CURSECOND = "\\$\\{CURSECOND\\}";        //当前秒
    private static String CURMILLISECOND = "\\$\\{CURMILLISECOND\\}";    //当前毫秒

    public static String macroReplace(String srcStr) {
        srcStr = dateMacroReplace(srcStr);
        return srcStr;
    }
     ...
     * @return
     */
    private static String dateMacroReplace(String srcStr) {
...
//        String srcStr = "${CURYEAR2}-${CURMONTH}-${CURDAY} ${CURHOUR}:${CURMINUTE}";
//        System.out.println(macroReplace(srcStr));
